...
const downloadsToday = computed(() => {
  ...
  return historyStore.downloadHistory.filter((photo) => {
...
<template>
  <aside class="w-80 bg-base-300 border-l border-base-100 p-3 overflow-y-auto">
    <div class="flex items-center justify-between mb-4">
      ...
      <button
        v-if="historyStore.downloadHistory.length"
        type="button"
        class="btn btn-ghost btn-xs btn-circle"
        @click="historyStore.clearDownloadHistory()"
      >
        <svg
    ...
    <div class="mb-4">
      ...
      >
        <div class="stat">
          <div class="stat-title text-xs opacity-70">Downloads today</div>
          <div class="stat-value text-lg font-medium font-outfit">
            {{ downloadsToday }}
          </div>
          <div
            class="stat-desc flex items-center justify-center gap-1 text-[10px] whitespace-normal"
          >
            <span
            ...
            >
              ...
              <span>
                {{ todayVsYesterday.delta > 0 ? '+' : ''
                }}{{ todayVsYesterday.percent }}%
              </span>
            </span>
            <span v-else class="opacity-60"> No data for yesterday </span>
          </div>
        </div>

        <div class="stat">
          <div class="stat-title text-xs opacity-70">Yesterday</div>
          <div class="stat-value text-lg font-medium font-outfit">
            {{ downloadsYesterday }}
          </div>
    ...
    <div
      v-if="historyStore.downloadHistory.length === 0"
    ...
    <div v-else class="space-y-4 pb-6">
      <div
        v-for="photo in historyStore.downloadHistory"
        :key="photo.id"
        class="flex gap-3 items-center"
      >
        <button
          type="button"
          class="relative group w-10 h-10 rounded overflow-hidden shrink-0"
        ...
        <div class="flex-1">
          <p class="text-sm font-medium truncate w-60">{{ photo.alt }}</p>
          <p class="text-xs opacity-60">
            At
            {{
              new Date(photo.downloadedAt || '').toLocaleString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
